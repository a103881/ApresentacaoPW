...
document.addEventListener("DOMContentLoaded", () => {
  const auditoriasPorCidade = JSON.parse(
    localStorage.getItem("auditoriasPorCidade")
  );
  ...
  if (userData) {
    ...
    if (nomePerfil) nomePerfil.textContent = userData.nome;
    if (emailPerfil) emailPerfil.textContent = userData.email;
    if (fotoPerfil) fotoPerfil.src = userData.foto;

    const userFoto = document.getElementById("userFoto");
    const userFotoBig = document.getElementById("userFotoBig");
    const userNome = document.getElementById("userNome");
    const userNomeBig = document.getElementById("userNomeBig");
    const userEmail = document.getElementById("userEmail");

    if (userFoto) userFoto.src = userData.foto;
    if (userFotoBig) userFotoBig.src = userData.foto;
    if (userNome) userNome.textContent = userData.nome;
    if (userNomeBig) userNomeBig.textContent = userData.nome;
    if (userEmail) userEmail.textContent = userData.email;
  } else {
    document.querySelector("body").innerHTML = `
    <div class="text-center text-red-600 mt-20">
      Utilizador não autenticado. <a href="login.html" class="underline text-[#037171]">Voltar ao login</a>
    </div>`;
  }
});
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (userData) {
    ...
    if (document.getElementById("userFoto"))
      document.getElementById("userFoto").src = foto;
    if (document.getElementById("userFotoBig"))
      document.getElementById("userFotoBig").src = foto;
    if (document.getElementById("userNome"))
      document.getElementById("userNome").textContent = nome;
    if (document.getElementById("userNomeBig"))
      document.getElementById("userNomeBig").textContent = nome;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (Profissionais && document.getElementById("IDprofissionaisServico")) {
    document.getElementById("IDprofissionaisServico").textContent =
      Profissionais;
  }
});
...
document.addEventListener("DOMContentLoaded", () => {
  const auditoriasPorCidade =
    JSON.parse(localStorage.getItem("auditoriasPorCidade")) || {};
  ...
  if (corpoTabela) {
    ...
    for (const cidade in auditoriasPorCidade) {
      ...
      row.innerHTML = `
		  ...
		  <td class="text-end">€${custoEstimado
        .toFixed(2)
